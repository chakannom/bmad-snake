...
- 핵심 범위: 모바일 터치 조작 + PC 키보드 조작, 20개 맵, 단계별 해금, 제한시간 내 목표점수 클리어, 재시작 버튼(방향패드 중앙 금지)
...
4. UI: HUD, state labels, restart/settings buttons
5. Persistence: localStorage (진행도, 설정)
...
## 4) 핵심 데이터 모델
- GameStatus: `idle | running | paused | gameover | clear`
...
## 5) 게임 규칙
- 맵 시작 시 타이머 시작
- 제한시간 내 목표점수 도달 시 `clear`
- 벽/장애물/자기몸 충돌 시 `gameover`
- clear 시 다음 맵 해금(최대 20)
- 역방향 입력 금지

## 6) 입력/컨트롤 설계
- PC: Arrow keys
- 모바일: 4방향 터치 버튼
- 재시작 버튼: 방향패드 블록 바깥(우측 상단 고정 권장)
- 버튼 최소 터치영역: 44x44 px
...
- 상태 전이(`idle/running/paused/gameover/clear`) 테스트 통과
