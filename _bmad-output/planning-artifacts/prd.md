...
초기 범위는 모바일 우선 조작 체계, 20개 맵 progression, 일관된 다크 테마 UI, 기본 게임 상태 관리(ready/running/paused/gameover/cleared)를 포함한다.

### What Makes This Special

이 제품의 차별점은 고정된 단일 기능이 아니라, 개발 과정에서 실험·검증을 통해 차별점을 점진적으로 형성하는 전략에 있다.
첫 번째 차별 축은 모바일 터치 조작 품질(방향 버튼 + D-pad 상단 좌측 START/RESTART 통합 버튼)이며, 두 번째 축은 20개 맵 난이도 곡선으로 구성된 progression 설계다.
...
- 방향 버튼 기반 터치 조작
- START/RESTART 통합 버튼 배치(D-pad 상단 좌측, 중앙 금지)
...
- 실수로 죽어도 START/RESTART 통합 버튼으로 빠르게 재도전한다.
...
- 상태 가시성(`ready/running/paused/gameover/cleared`)
...
- 터치 입력 신뢰성 최우선(오입력 최소화, START/RESTART 버튼 오배치 금지)
- 모바일 성능 기준: 평균 55fps 이상 유지
- 상태 전이 안정성: `ready/running/paused/gameover/cleared` 일관된 상태머신 관리
...
- 상태 관리: 게임 상태(`ready/running/paused/gameover/cleared`)와 UI 상태를 분리
...
- 터치 버튼 44px+ 기준 유지, START/RESTART 버튼은 방향키 중앙 배치 금지
...
- 맵 20개는 데이터 기반 구조(`StageConfig[]`)로 분리
...
- 모바일 터치 방향 입력
- D-pad 상단 좌측의 START/RESTART 통합 버튼
...
- FR3: 시스템은 게임 상태(`ready/running/paused/gameover/cleared`)를 일관되게 전이·유지할 수 있다.
...
- FR9: 시스템은 START/RESTART 통합 버튼을 D-pad 상단 좌측(중앙 제외)에 제공할 수 있다.

### 맵/진행 시스템

- FR10: 시스템은 20개 맵 정의를 로드할 수 있다.
- FR11: 플레이어는 해금된 맵 중 선택한 맵에서 플레이를 시작할 수 있다.
...
### 접근성
- FR19: 시스템은 터치 방향 버튼의 최소 터치 영역을 44px x 44px 이상으로 제공하고, 모바일 테스트 세션에서 오입력률을 5% 이하로 유지할 수 있다.
- FR20: 시스템은 `ready/running/paused/gameover/cleared` 상태 각각에 대응하는 상태 표시 텍스트를 캔버스 상단에 표시하고, 상태 변경 후 100ms 이내에 업데이트할 수 있다.
- FR21: 시스템은 주요 인터랙션 요소(시작/일시정지/재시작/방향버튼)에 접근성 레이블을 제공하고, 키보드 포커스 이동이 논리 순서대로 동작할 수 있다.
...
- NFR-Performance-4: FPS/입력 p95 HUD는 PC 환경에서만 표시하며, 모바일 환경에서는 HUD 표시 없이 동일 지표를 검증 로그로 수집한다.

### Reliability

- NFR-Reliability-1: 치명 크래시율은 내부 테스트 200세션 기준 1% 미만이어야 하며, 세션 로그로 측정한다.
- NFR-Reliability-2: 게임 상태 전이는 정의된 상태머신(`ready/running/paused/gameover/cleared`) 전이표 100%를 통과해야 하며, 자동 상태 전이 테스트로 검증한다.
...
- NFR-Security-1: MVP 범위에서는 민감 개인정보를 저장/처리하지 않아야 하며, 저장 데이터는 로컬 게임 진행 데이터로 제한한다.
